...
## Security Guardrails
Automated checks (closing Findings #7 and #8 of the April 2026 security review) block PRs and tag releases when known-bad patterns reappear. The single source of truth is `scripts/security-guardrails.sh`. It enforces five invariants:
...
5. No `https://` / `http://` / protocol-relative URL in `mv3-extension/` JS/CSS/HTML references a host outside the `ALLOWED_REMOTE_HOSTS` allowlist. Vendor the asset, or add the host (with a one-line rationale) to the allowlist constant in `scripts/security-guardrails.sh`.
